...
query = "SELECT SESSION_CONTEXT('XS_APPLICATIONUSER') \"APPLICATION_USER\" FROM \"DUMMY\"";  
rs = await conn.executeQuery(query);
var xsApplicationUser = rs[0].APPLICATION_USER;

var greeting = {
    XS_Layer_Session_User: $.session.getUsername(), 
	Database_Current_User: currentUser, 
	Database_Application_User: applicationUser, 
	XS_Database_Application_User: xsApplicationUser
};
